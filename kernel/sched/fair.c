...
#define SCHED_FLAG_DL_OVERRUN 0x0
#endif

#ifndef SCHED_FLAG_RECLAIM
#define SCHED_FLAG_RECLAIM 0x0
#endif

static inline bool sched_core_cookie_match(struct rq *rq, struct task_struct *p) { return true; }
static inline void dl_server_update(struct sched_dl_entity *dl_se, s64 delta_exec) {}
static inline void dl_server_start(struct sched_dl_entity *dl_se) {}
static inline void dl_server_stop(struct sched_dl_entity *dl_se) {}
static inline void dl_server_init(struct sched_dl_entity *dl_se, struct rq *rq,
		    struct task_struct * (*pick)(struct sched_dl_entity *, struct rq_flags *)) {}
static inline void sched_init_dl_servers(struct rq *rq) {}

static inline bool sched_energy_enabled(void) { return energy_aware(); }
static inline bool task_current_donor(struct rq *rq, struct task_struct *p) { return task_current(rq, p); }
static inline bool task_has_idle_policy(struct task_struct *p) { return idle_policy(p->policy); }
static inline bool normal_policy(int policy) { return fair_policy(policy); }

/* 4.14 Backport: Missing sysctls used by core/sysctl */
unsigned int sysctl_sched_latency			= 6000000ULL;
unsigned int sysctl_sched_min_granularity		= 750000ULL;
unsigned int sysctl_sched_wakeup_granularity		= 1000000ULL;
unsigned int sysctl_sched_child_runs_first		= 1;

/* 4.14 Backport: sched_slice used by core */
...
/* 4.14 Wrappers for 6.19 EEVDF backport */

static void set_curr_task_fair_wrapper(struct rq *rq)
{
	set_next_task_fair(rq, rq->curr, false);
}

static void put_prev_task_fair_wrapper(struct rq *rq, struct task_struct *prev)
{
	put_prev_task_fair(rq, prev, NULL);
}

#ifdef CONFIG_SMP
static int select_task_rq_fair_wrapper(struct task_struct *p, int task_cpu, int sd_flag, int flags,
			       int subling_count_hint)
{
    /* 6.19 uses wake_flags. We pass flags. */
    return select_task_rq_fair(p, task_cpu, flags);
}

static void migrate_task_rq_fair_wrapper(struct task_struct *p)
{
    migrate_task_rq_fair(p, task_cpu(p));
}

static void set_cpus_allowed_fair_wrapper(struct task_struct *p, const struct cpumask *newmask)
{
...
const struct sched_class fair_sched_class = {
	...
	.put_prev_task		= put_prev_task_fair_wrapper,
	.set_curr_task          = set_curr_task_fair_wrapper,

#ifdef CONFIG_SMP
	.select_task_rq		= select_task_rq_fair_wrapper,
	.migrate_task_rq	= migrate_task_rq_fair_wrapper,
	.rq_online		= rq_online_fair,
	.rq_offline		= rq_offline_fair,
	.task_woken		= NULL,
    .set_cpus_allowed   = set_cpus_allowed_fair_wrapper,
#endif
